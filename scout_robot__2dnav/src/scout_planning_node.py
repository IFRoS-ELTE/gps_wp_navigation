...
class OnlinePlanner:
    ...
    def __init__(
        ...
        self.svc = StateValidityChecker(
            distance_threshold, is_unknown_valid, is_rrt_star
        )
        ...
        self.move_goal_sub = rospy.Subscriber(
            "/move_base_simple/goal", PoseStamped, self.get_goal, queue_size=1
        )
        self.cmd = Twist()
        # TIMERS
    ...
    def recovery_behavior(self):
        ...
        psi_d = math.atan2(
            pose[1] - self.current_pose[1], pose[0] - self.current_pose[0]
        )
    ...
    def get_gridmap(self, gridmap):
        ...
        if (gridmap.header.stamp - self.last_map_time).to_sec() > 1:
            ...
            env = (
                np.array(gridmap.data)
                .reshape(gridmap.info.height, gridmap.info.width)
                .T
            )
            ...
            # If the robot is not following a path, but there is a goal, plan a new path to the goal
                ...
                and self.svc.is_valid(self.goal)
                and len(self.path) == 0
            ):
    ...
    def plan(self):
        ...
        # TODO: plan a path from self.current_pose to self.goal
            ...
            self.path, self.edges = compute_path(
                self.current_pose, self.goal, self.svc, self.bounds
            )
            self.draw_tree()
...
if __name__ == "__main__":
    rospy.init_node("scout_planning_node")
    is_rrt_star = False
